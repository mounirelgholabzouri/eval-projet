...
14. **Logo + Tampon impression (navigateur)** : toujours intégrer les images critiques en base64 (`base64_encode(file_get_contents($path))`). Le tampon `assets/img/tampon_ofppt.png` doit être placé **hors du div `.page`** (juste avant `</body>`) avec `position: fixed; bottom: 14mm; right: 14mm; width: 38mm` — `position: fixed` se répète sur chaque page imprimée. Ne jamais utiliser `position: absolute` dans un parent `position: relative` (annule `fixed`).
17. **Tampon impression PDF mPDF** (`admin/generate_efm_pdf.php`) : mPDF **ne supporte pas `right` ni `bottom`** en `position: fixed` → utiliser `$mpdf->SetHTMLFooter()` avec image alignée à droite + `margin_bottom: 48` + `margin_footer: 8`. Le tampon apparaît ainsi en bas à droite de chaque page PDF générée.
